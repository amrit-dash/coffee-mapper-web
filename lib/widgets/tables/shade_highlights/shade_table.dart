...
class ShadeTable extends StatefulWidget {
  final List<ShadeData> shadeData;
  final bool isLoggedIn;
  final Function(ShadeData)? onDelete;
  ...
  @override
  State<ShadeTable> createState() => _ShadeTableState();
}

class _ShadeTableState extends State<ShadeTable> {
            ...
            child: ScrollConfiguration(
              ...
              child: DataTable2(
                ...
                columns: [
                  ...
                  ...TableColumns.shadeColumns.map((def) => DataColumn2(
                    label: _buildColumnLabel(context, def.label),
                    size: def.size,
                    fixedWidth: ResponsiveUtils.getColumnWidth(screenWidth, def.width),
                  )),
                ],
                rows: widget.shadeData.map((data) => _buildDataRow(context, data)).toList(),
              ),
            ),
          ),
  ...
  DataRow _buildDataRow(BuildContext context, ShadeData data) {
    List<DataCell> cells = [];
  ...
  Future<void> _showDeleteDialog(BuildContext context, ShadeData data) async {
    return showDialog(
      context: context,
